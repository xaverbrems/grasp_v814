...
switch todo
    ...
    case 'export_2d_data'
                ...
                if findstr(inst_component(n).name,'Detector') %Find detector component
                    for m = 1:inst_component(n).pannels %Loop though detector pannels
                        ...
                        if isfield(pannel_struct,'detector_data') %check that there is some data to export
                            numor_data.wav = inst_config.mono_wav; %wav
                            numor_data.dwav = inst_config.mono_dwav/100; %delta_wav
                            numor_data.det = inst_component(n).position; %det
                            numor_data.det_calc = inst_component(n).position; %det_calc
                            numor_data.col = inst_config.col; %col
                            numor_data.time = sans_instrument_model_params.measurement_time *10; %time 10ths seconds 10 = 1second
                            numor_data.monitor = sans_instrument_model_params.monitor(1); %Beam monitor
                            numor_data.pixel_size = [pannel_struct.parameters.pixel_size(1)*1e3, pannel_struct.parameters.pixel_size(2)*1e3];
                            numor_data.subtitle = sans_instrument_model_params.subtitle;
                            numor_data.source_size = inst_config.source_size; % source size
                            ...
                            directory = [sans_instrument_model_params.data_dir filesep];
                            ill_nexus_write(directory,numor,numor_data,'D22');
        ...
        elseif strcmp(inst_config.inst,'ILL_d33')
            ...
            numor_data.detectors = det_counter;
            
            
            %Write the D33 NEXUS file
            directory = [sans_instrument_model_params.data_dir];
            ill_nexus_write(directory,numor,numor_data,'D33');
        ...
        %                         data.(['params' num2str(det)])(3,:) = sans_instrument_model_params.measurement_time; %time seconds
        %
        %                         data.(['params' num2str(det)])(5,:) = sans_instrument_model_params.monitor; %beam monitor
        ...
        %             data.subtitle = sans_instrument_model_params.subtitle;
        %             data.numor = numor;
    ...
    case 'plot_2D'
        ...
        qdepth = [];
        if strcmp(sans_instrument_model_params.det_image,'Mod_q'); qdepth = 5;
        elseif strcmp(sans_instrument_model_params.det_image,'Mod_q'); qdepth = 5;
        elseif strcmp(sans_instrument_model_params.det_image,'q_x'); qdepth = 3;
        elseif strcmp(sans_instrument_model_params.det_image,'q_y'); qdepth = 4;
        elseif strcmp(sans_instrument_model_params.det_image,'q_Angle'); qdepth = 6;
        elseif strcmp(sans_instrument_model_params.det_image,'Solid_Angle'); qdepth = 10;
        end
