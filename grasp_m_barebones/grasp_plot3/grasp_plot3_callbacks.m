function grasp_plot3_callbacks(to_do,option)
...
switch to_do
    ...
    case 'curve_math'
        ...
        if isempty(h1dat)
            ...
            temp = strfind(plot_info1.column_format,'h');
            if ~isempty(temp) 
        ...
        grasp_plot3(plot_info1.plot_data(:,1),plot_info1.plot_data(:,2),plot_info1.plot_data(:,3),....
            'columnformat',plot_info1.column_format,'title',plot_info1.plot_title,....
            'xlabel',plot_info1.xlabel,....
            'ylabel','Curve Math',....
            'legend',{plot_info1.legend_str});
    ...
    case 'erase_curve'
        ...
        grasp_plot3_callbacks('update_legend',grasp_plot_handles);
        grasp_plot3_fit_callbacks('build_curve_number');
        grasp_plot3_callbacks('build_curve_tools_menu',grasp_plot_handles);
    ...
    case 'build_curve_tools_menu'
        ...
        if length(curve_handles) <=20 %Do not make Curve tools menu if there are too many curves
            for n = 1:length(curve_handles)
                if not(isempty(curve_handles{n}))
                    ...
                    uimenu(handle,'label','Erase Curve','userdata',curve,'callback','grasp_plot3_callbacks(''erase_curve'');');
                    uimenu(handle,'label','Math on Curve','userdata',curve,'callback','grasp_plot3_callbacks(''math_expression_curve'');');
                    ...
                    for math = 1:length(mathops)
                        ...
                        for n2 = 1:length(curve_handles)
                            if not(isempty(curve_handles{n2}))
                                ...
                                uimenu(handle3,'label',['Curve: ' num2str(curve2)],'userdata',userdata,'callback','grasp_plot3_callbacks(''curve_math'')');
                            end
                        end
                    end
                end
        ...
        handle = uimenu(option.curve_tools_menu,'label',['Average All Curves'],'callback','grasp_plot3_callbacks(''average_curves'');');
    ...
    case 'update_legend'
        ...
        if not(isempty(legend_curve_handle))
            handle = legend(legend_curve_handle,legend_str,'textcolor','white');
    ...
    case 'ax_lim_check'
        ...
        parent_figure = get(gcbo,'parent');
        grasp_plot_handles = get(parent_figure,'userdata');
        grasp_plot_handles.ax_lim_check = status_str;
        set(parent_figure,'userdata',grasp_plot_handles);
        
        set(grasp_plot_handles.ymax,'visible',status_str);
        set(grasp_plot_handles.ymin,'visible',status_str);
        set(grasp_plot_handles.xmax,'visible',status_str);
        set(grasp_plot_handles.xmin,'visible',status_str);
