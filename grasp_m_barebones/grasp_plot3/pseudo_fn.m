...
if graspplot.fitter.include_res_check ==1  %Convolute instrument resolution if availaible
    ...
    elseif isfield(fitdata,'hdat_all') %Convolute to Gaussian resolution if full shape kernel no availaible
        ...
        if not(isempty(fitdata.hdat_all))
            ...
            fwhmwidth = graspplot.resolution_control.fwhmwidth;
            finesse = graspplot.resolution_control.finesse * graspplot.resolution_control.fwhmwidth;
            for n = 1:length(x_in)
...
for fn_multiplex = 1:graspplot.fitter.number1d
    ...
    for variable_loop = 1:graspplot.fitter.function_info_1d.no_parameters
        ...
        eval([graspplot.fitter.function_info_1d.variable_names{param_number} ' = ' num2str(parameters_in(param_number)) ';']);
        param_number = param_number+1;
    end
    for line = 1:length(graspplot.fitter.function_info_1d.math_code)
        %graspplot.fitter.function_info_1d.math_code{line}
        eval(graspplot.fitter.function_info_1d.math_code{line});%this takes 'x' and gives a variable called 'y' as the result
    end
