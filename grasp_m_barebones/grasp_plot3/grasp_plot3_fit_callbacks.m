function output = grasp_plot3_fit_callbacks(to_do,option)
...
font = graspplot.font; fontsize = graspplot.fontsize;
...
switch(to_do)
    ...
    case 'delete_onoff_check'
        graspplot.fitter.delete_curves_check = get(gcbo,'value');
    ...
    case 'include_resolution_check'
        graspplot.fitter.include_res_check = get(gcbo,'value');
        grasp_plot3_fit_callbacks('draw_fn',guess_color); %update the function
    ...
    case 'simultaneous_fit_check'
        graspplot.fitter.simultaneous_check = get(gcbo,'value');
        grasp_plot3_fit_callbacks('draw_fn',guess_color); %update the function
    ...
    case 'auto_guess'
        ...
        if strcmp(option,'point_click')
            code = graspplot.fitter.function_info_1d.point_click_code;
        else
            code = graspplot.fitter.function_info_1d.auto_guess_code;
            if graspplot.fitter.number1d >1
                disp(' ');
        ...
        if not(isempty(code))
            ...
            try
                ...
                for n = 1:length(final_guess)
                    if graspplot.fitter.function_info_1d.fix(n) ~=1 %i.e. not fixed
                        graspplot.fitter.function_info_1d.values(n) = final_guess(n);
                        graspplot.fitter.function_info_1d.err_values(n) = 0;
                    end
                end
                
                grasp_plot3_fit_callbacks('update_curve_fit_window');
                grasp_plot3_fit_callbacks('draw_fn',guess_color);
    ...
    case 'copy_to_clipboard'
        ...
        str = [];
        for n = 1:length(graspplot.fitter.function_info_1d.values)
            str = [str graspplot.fitter.function_info_1d.long_names{n} char(9) num2str(graspplot.fitter.function_info_1d.values(n),'%0.5g') char(9) num2str(graspplot.fitter.function_info_1d.err_values(n),'%0.5g') char(13) char(10)];
        end
        clipboard('copy',str);
    ...
    case 'build_curve_number'
        ...
        try
        if isfield(graspplot_handles.curve_fit1d,'curve_number')
            if ishandle(graspplot_handles.curve_fit1d.curve_number)
                set(graspplot_handles.curve_fit1d.curve_number,'string',n_str);
                value = get(graspplot_handles.curve_fit1d.curve_number,'value');
                if value > length(n_str)
                    set(graspplot_handles.curve_fit1d.curve_number,'value',length(n_str));
                end
    ...
    case 'build_fn_list'
        ...
        graspplot.fitter.fn_list1d = fn_name_list; %Keep a store of the fitting functions in the status_flags
        try val = graspplot.fitter.fn1d; catch val = 1; end
        set(graspplot_handles.curve_fit1d.fn_selector,'string',fn_name_list,'value',val);
        
        
    case 'retrieve_fn'
        
        if isempty(option)
            if graspplot.fitter.fn1d > length(graspplot.fitter.fn_list1d); graspplot.fitter.fn1d = 1;  close(graspplot_handles.curve_fit1d.window); grasp_plot_fit_window; end
            fn.name = graspplot.fitter.fn_list1d{graspplot.fitter.fn1d};
        else
            fn.name = option;
        end
        fn.variable_names = [];
        ...
        if not(fid==-1)
            ...
            if not(isempty(strfind(fn.name,'----'))) %Check for Divider line
                graspplot.fitter.function_info_1d = fn; %Put the blank functions parameters in to status flags
                grasp_plot3_fit_callbacks('update_curve_fit_window'); %update the window to be blank
                return;
            end
        ...
        graspplot.fitter.function_info_1d = fn; %some of these get added to in the multiplex process below
        
        %***** Function Multiplex *****
        %Multiply up the function i.e. n of the same functions, with
        %different or grouped parameters added together
        for n = 2:graspplot.fitter.number1d
            graspplot.fitter.function_info_1d.fix = [graspplot.fitter.function_info_1d.fix, fn.fix];
            graspplot.fitter.function_info_1d.group = [graspplot.fitter.function_info_1d.group, fn.group];
            graspplot.fitter.function_info_1d.values = [graspplot.fitter.function_info_1d.values, fn.values];
            graspplot.fitter.function_info_1d.err_values = [graspplot.fitter.function_info_1d.err_values, fn.err_values];
            graspplot.fitter.function_info_1d.variable_names = [graspplot.fitter.function_info_1d.variable_names, fn.variable_names];
            graspplot.fitter.function_info_1d.long_names = [graspplot.fitter.function_info_1d.long_names, fn.long_names];
            %graspplot.fitter.function_info_1d.point_click_code = [graspplot.fitter.function_info_1d.point_click_code, fn.point_click_code];
        end
        
        %Also allow output of the fit function (e.g. for Ancos2)
        output = graspplot.fitter.function_info_1d;
        
        
    case 'update_curve_fit_window'
        
        %Delete old displayed parameters
        if isfield(graspplot_handles,'fitter')
            if isfield(graspplot_handles.fitter,'fit1d_handles')
                for n = 1:length(graspplot_handles.fitter.fit1d_handles)
                    if ishandle(graspplot_handles.fitter.fit1d_handles(n)); delete(graspplot_handles.fitter.fit1d_handles(n)); end
                end
            end
        end
        %Draw parameter entry boxes and check-boxes - dependent on the number and parameter names returned by the function
        graspplot_handles.fitter.fit1d_handles = [];
        handles_store = [];
        for n = 1:length(graspplot.fitter.function_info_1d.long_names)
            %Parameter Names
            handle = uicontrol(graspplot_handles.curve_fit1d.window, 'units','normalized','Position',[0.02,(0.8-(n*0.03)),0.3, 0.028],'FontName',font,'FontSize',fontsize,'HorizontalAlignment','right','Style','text','String',[graspplot.fitter.function_info_1d.long_names{n} ': ' graspplot.fitter.function_info_1d.variable_names{n} ' : ' ],'BackgroundColor', background_color, 'ForegroundColor', [1 1 1]);
            handles_store = [handles_store, handle];
            %Fix Check
            handle = uicontrol(graspplot_handles.curve_fit1d.window, 'units','normalized','Position',[0.37,(0.8-(n*0.03)),0.038,0.028],'ToolTip',['Fix ' graspplot.fitter.function_info_1d.long_names{n}],'FontName',font,'FontSize',fontsize,'Style','checkbox','HorizontalAlignment','left','backgroundcolor',background_color,'value',graspplot.fitter.function_info_1d.fix(n),'userdata',n,'callback','grasp_plot3_fit_callbacks(''fn_value_fix_check'');');
            handles_store = [handles_store, handle];
            
            if graspplot.fitter.function_info_1d.group(n) == 1 && n > graspplot.fitter.function_info_1d.no_parameters
                visible = 'off';
            else
                visible = 'on';
            end
            %Parameter Value
            handle = uicontrol(graspplot_handles.curve_fit1d.window, 'visible',visible, 'units','normalized','Position',[0.45,(0.8-(n*0.03)),0.15,0.028],'FontName',font,'FontSize',fontsize,'Style','edit','HorizontalAlignment','left','string',num2str(graspplot.fitter.function_info_1d.values(n)),'userdata',n,'callback','grasp_plot3_fit_callbacks(''fn_value_enter'');');
            handles_store = [handles_store, handle];
            %Parameter Value Error
            handle = uicontrol(graspplot_handles.curve_fit1d.window, 'visible',visible, 'units','normalized','Position',[0.62,(0.8-(n*0.03)),0.15,0.028],'FontName',font,'FontSize',fontsize,'Style','text','HorizontalAlignment','left','string',num2str(graspplot.fitter.function_info_1d.err_values(n)));
            handles_store = [handles_store, handle];
            if n <= graspplot.fitter.function_info_1d.no_parameters
                %Group Check
                handle = uicontrol(graspplot_handles.curve_fit1d.window, 'units','normalized','Position',[0.81,(0.8-(n*0.03)),0.038,0.028],'ToolTip',['Group ' graspplot.fitter.function_info_1d.long_names{n} '`s'],'FontName',font,'FontSize',fontsize,'Style','checkbox','HorizontalAlignment','left','backgroundcolor',background_color,'value',graspplot.fitter.function_info_1d.group(n),'userdata',n,'callback','grasp_plot3_fit_callbacks(''fn_group_check'');');
                handles_store = [handles_store, handle];
            end
        end
        handle = uicontrol(graspplot_handles.curve_fit1d.window, 'units','normalized','Position',[0.02,0.1,0.96,0.05],'FontName',font,'FontSize',fontsize,'Style','text','HorizontalAlignment','left','backgroundcolor',background_color,'foregroundcolor','white','string',graspplot.fitter.function_info_1d.math_code);
        handles_store = [handles_store, handle];
        
        graspplot_handles.fitter.fit1d_handles = handles_store;
    ...
    case 'draw_fn'
        ...
        fitdata = grasp_plot3_fit_callbacks('get_fit_data'); %fitdata.curve_number contains the curve number
...
%             end
            ...
            %Generate function
            y = pseudo_fn_gp3(x,graspplot.fitter.function_info_1d.values,extra);
            
            
            %Delete old guess curve
            plot_info = get(curve_handles{fitdata.curve_number(n)}(1),'userdata');
            if graspplot.fitter.delete_curves_check == 0
                if isfield(plot_info,'fit_curve_handle')
    ...
    case 'toggle_fn'
        graspplot.fitter.fn1d = get(gcbo,'value');
        grasp_plot3_fit_callbacks('retrieve_fn');
        grasp_plot3_fit_callbacks('update_curve_fit_window');
    ...
    case 'fn_value_enter'
        ...
        if not(isempty(value))
            graspplot.fitter.function_info_1d.values(1,param_number) = value;
        end
        grasp_plot3_fit_callbacks('draw_fn',guess_color); %update the function
        
    case 'fn_value_fix_check'
        param_number = get(gcbo,'userdata');
        graspplot.fitter.function_info_1d.fix(param_number) = get(gcbo,'value');
        
    case 'fn_group_check'
        param_number = get(gcbo,'userdata');
        graspplot.fitter.function_info_1d.group(param_number) = get(gcbo,'value');
        while param_number <= length(graspplot.fitter.function_info_1d.group);
            graspplot.fitter.function_info_1d.group(param_number) = get(gcbo,'value');
            %also set the fix check for the grouped copies
            if param_number > graspplot.fitter.function_info_1d.no_parameters;
                graspplot.fitter.function_info_1d.fix(param_number) = 1;
            end
            param_number = param_number + graspplot.fitter.function_info_1d.no_parameters;
        end
        grasp_plot3_fit_callbacks('update_curve_fit_window');
        
    case 'number_of_fn'
        graspplot.fitter.number1d = get(gcbo,'value');
        grasp_plot3_fit_callbacks('retrieve_fn');
        grasp_plot3_fit_callbacks('update_curve_fit_window');
    ...
    case 'get_fit_data'
        ...
        if graspplot.fitter.simultaneous_check == 1 %fit all curves
            n_curves = length(curve_handles);
            curve = 1:n_curves;
            %Make empty arrays
            for n = 1:n_curves
            ...
            value = get(graspplot_handles.curve_fit1d.curve_number,'value');
            str = get(graspplot_handles.curve_fit1d.curve_number,'string');
      ...
      %***** Check for any zero errors coming though. These need to be allocated someting finite for the fitter to work *****
        ...
        for n = 1:n_curves
            ...
            if no_points <10
                ...
                disp('Help:  What to do in grasp_plot3_fit_callbacks')
...
%        message_handle = grasp_message(['Working: 1D Curve Fitting'],1,'sub');
        ...
        fitdata = grasp_plot3_fit_callbacks('get_fit_data'); %get fit data and curve number
        
        %Fit it!. - Using M_Fit
        fun_name = 'pseudo_fn_gp3';
        
        start_params = graspplot.fitter.function_info_1d.values;
        ...
        vary_mask = double(not(graspplot.fitter.function_info_1d.fix));
        ...
        if sum(vary_params)~=0
            ...
            %Run the fit once
            [fit_params, fit_params_err,chi2] = mf_lsqr_gp3(fitdata.xdat_all,fitdata.ydat_all,fitdata.edat_all,start_params,vary_params,fun_name,fitdata);
            ...
            for n = 1:length(params_to_vary)
                ...
                [~, temp_fit_params_err] = mf_lsqr_gp3(fitdata.xdat_all,fitdata.ydat_all,fitdata.edat_all,fit_params,temp_vary_params,fun_name,fitdata);
            ...
            graspplot.fitter.function_info_1d.values = fit_params';
            graspplot.fitter.function_info_1d.err_values = fit_params_err_cov';
            
            %make final copy of any grouped parameters into theircopy positions
            param_number = 1;
            for fn_multiplex = 1:graspplot.fitter.number1d
                for variable_loop = 1:graspplot.fitter.function_info_1d.no_parameters
                    if graspplot.fitter.function_info_1d.group(param_number) == 1
                        graspplot.fitter.function_info_1d.values(param_number) = graspplot.fitter.function_info_1d.values(variable_loop);
                        graspplot.fitter.function_info_1d.err_values(param_number) = graspplot.fitter.function_info_1d.err_values(variable_loop);
                    end
            ...
            l = length(graspplot.fitter.function_info_1d.long_names);
            for n = 1:l(1)
                disp([graspplot.fitter.function_info_1d.long_names{n} ' : ' num2str(graspplot.fitter.function_info_1d.values(n)) '  err: ' num2str(graspplot.fitter.function_info_1d.err_values(n))]);
            end
            ...
            fit_parameters = graspplot.fitter.function_info_1d;
            fit_parameters.number_functions = graspplot.fitter.number1d;
            
            %Add Fit History to the displayimage structure
            fit_history = {['Curve Fit:' graspplot.fitter.function_info_1d.name]};
            l = length(graspplot.fitter.function_info_1d.variable_names);
            for n = 1:l(1)
                fit_history(length(fit_history)+1,:) = {[graspplot.fitter.function_info_1d.variable_names{n} ' : ' num2str(fit_parameters.values(n)) ' err: ' num2str(fit_parameters.err_values(n))]};
            end
...
%        delete(message_handle);
        ...
        grasp_plot3_fit_callbacks('draw_fn',fit_color); %update the function
